...
export class UsersService {
  ...
  async findById(id: number) {
    ...
    for (const userBadge of user.usersBadges) {
      ...
      try {
        ...
        const url = await getSignedUrl(this.s3Service.getS3Client(), command, {
          expiresIn: 600,
        });
        userBadge.badge.imageUrl = url;
      } catch (error) {
        console.error(
          `Error to get signedUrl ${key} from aws: ${error.message}`
        );
      }
    }
  ...
  async create(createUserDTO: CreateUserDTO) {
    ...
    try {
      ...
      await this.usersExercisesService.addToNewUserInitalExercises(newUser);
      await this.usersBadgesService.addToNewUserInitialBadges(newUser);
      await this.usersTechProgressService.addToNewUserInitialTechProgress(
        newUser
      );
      return;
    } catch (err) {
  ...
  async updateXpPoints(userExercise: UserExercise) {
    ...
    await this.userRepository
      ...
      .execute();
    if (
      user.xpPoints + userExercise.exercise.xpToWin >
      rankProgress.requiredXpToUpdate
    ) {
      await this.userRepository
        .createQueryBuilder()
        .update()
